(function () {
  ...
  function localizedTierName(tier, locale) {
    const labels = TIER_LABELS[normalizeLocale(locale)] || {};
  ...
  async function buildShareImage(achievement) {
    ...
    // Tier badge pill
    const badgeLabel = achievement.tier_label || (tier.toUpperCase() + " TIER");
  ...
  function ShareDialog({ achievement, onClose }) {
    ...
    function tweetText() {
      const tierPart = achievement.tier ? ((achievement.tier_label || achievement.tier) + " ") : "";
  ...
  function AchievementCard({ achievement }) {
    ...
    let tierLabel;
    if (achievement.tier) {
      tierLabel = achievement.tier_label || achievement.tier;
    } else if (targetTier) {
      tierLabel = tx(t, "tier.target", "Target {tier}", { tier: achievement.next_tier_label || targetTier });
  ...
  function AchievementsPage() {
    ...
    hooks.useEffect(function () { load(); }, [locale]);
    ...
    const highestAchievement = highestRaw ? unlocked.find(function (a) { return a.tier === highestRaw; }) : null;
    const highest = highestAchievement ? (highestAchievement.tier_label || highestRaw) : tx(t, "stats.none_yet", "None yet");
