...
def unregister_server(name: str) -> bool:
    """Stop and remove a single MCP server by name.

    Schedules shutdown on the MCP background loop and waits for clean
    teardown.  Returns ``True`` if the server was found and shut down,
    ``False`` if it was not in the registry.
    """
    if not _MCP_AVAILABLE:
        return False
    ...
    if loop is not None and loop.is_running():
        ...
        future = safe_schedule_threadsafe(
            _shutdown(), loop,
            logger=logger,
            log_message=f"MCP unregister '{name}': failed to schedule",
        )
        if future is not None:
            try:
                future.result(timeout=15)
            except Exception as exc:
                logger.debug("Error during MCP unregister '%s': %s", name, exc)
    else:
        # Fallback when MCP background loop is not running.
        try:
            asyncio.run(_shutdown())
        except RuntimeError:
            pass

    # Clear circuit-breaker state so a future re-registration starts fresh.
    _server_error_counts.pop(name, None)
    _server_breaker_opened_at.pop(name, None)
    return True


def stop_servers_by_prefix(prefix: str) -> List[str]:
    """Stop all MCP servers whose names start with *prefix*.

    Returns the list of server names that were stopped.
    """
    if not _MCP_AVAILABLE:
        return []

    with _lock:
        names = [n for n in _servers if n.startswith(prefix)]

    stopped: List[str] = []
    for name in names:
        if unregister_server(name):
            stopped.append(name)
    return stopped


def register_dynamic_server(name: str, config: dict) -> List[str]:
    """Register a single MCP server at runtime (e.g. from a plugin).

    Thin wrapper around :func:`register_mcp_servers` for convenience.
    Returns the list of tool names registered from the server.
    """
    return register_mcp_servers({name: config})
