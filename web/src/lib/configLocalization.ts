...
import {
  ZH_CONFIG_PATH_DESCRIPTIONS,
  ZH_CONFIG_PATH_LABELS,
  ZH_CONFIG_SELECT_OPTIONS,
} from "../i18n/config-fields-zh";
...
const ZH_LABEL_OVERRIDES: Record<string, string> = {
  ...
  paste_collapse_threshold: "粘贴折叠阈值",
};
...
function humanizeKey(input: string): string {
  return normalizeLookupKey(input).replace(/_/g, " ");
}

function buildAutoDescriptionFromSchemaKey(schemaKey: string): string {
  return schemaKey
    .split(".")
    .map((segment) =>
...
function isAutoGeneratedSchemaDescription(
  schemaKey: string,
  description: string,
): boolean {
  const trimmed = description.trim();
  if (!trimmed) return false;
  return trimmed === buildAutoDescriptionFromSchemaKey(schemaKey);
}

function localizeConfigSegment(
  segment: string,
  locale: string | undefined,
): string {
  const normalized = normalizeLookupKey(segment);
  if (!isZh(locale)) return toEnglishLabel(segment);
  if (ZH_LABEL_OVERRIDES[segment]) return ZH_LABEL_OVERRIDES[segment];
  if (ZH_LABEL_OVERRIDES[normalized]) return ZH_LABEL_OVERRIDES[normalized];
  const compound = translateCompoundToken(segment);
  if (compound) return compound;
  return localizePhrase(humanizeKey(segment), locale);
}

function localizeSchemaKeyBreadcrumb(
  schemaKey: string,
  locale: string | undefined,
): string {
  return schemaKey
    .split(".")
    .map((segment) => localizeConfigSegment(segment, locale))
    .join(" → ");
}
...
export function getConfigSelectOptions(
  ...
  const normalizedSchemaKey = normalizeLookupKey(schemaKey);
  const options =
    ZH_SELECT_OPTION_OVERRIDES[schemaKey] ??
    ZH_SELECT_OPTION_OVERRIDES[normalizedSchemaKey];
  if (!options) return undefined;
  return Object.keys(options);
}

function toEnglishLabel(input: string): string {
  return humanizeKey(input)
    .replace(/_/g, " ")
    .replace(/\b\w/g, (char) => char.toUpperCase());
}

function translateCompoundToken(
  token: string,
): string | null {
  ...
  if (!pieces.every((piece) => ZH_TOKEN_OVERRIDES[piece])) return null;
  return pieces.map((piece) => ZH_TOKEN_OVERRIDES[piece]).join("");
}
...
function localizePhrase(phrase: string, locale: string | undefined): string {
  ...
  return trimmed
    ...
    .map((part) => {
      ...
      const direct =
        ZH_TOKEN_OVERRIDES[normalized] ?? ZH_TOKEN_OVERRIDES[part.toLowerCase()];
      if (direct) return direct;
      const compound = translateCompoundToken(part);
      if (compound) return compound;
...
export function localizeConfigCategory(
  ...
  if (categories[normalizedCategory]) return categories[normalizedCategory];
  if (isZh(locale) && ZH_CATEGORY_OVERRIDES[normalizedCategory]) {
    return ZH_CATEGORY_OVERRIDES[normalizedCategory];
  }
  return toEnglishLabel(normalizedCategory || category);
}
...
export function localizeConfigFieldLabel(
  ...
  if (isZh(locale) && ZH_CONFIG_PATH_LABELS[schemaKey]) {
    return ZH_CONFIG_PATH_LABELS[schemaKey];
  }
  ...
  if (isZh(locale) && ZH_LABEL_OVERRIDES[normalizedLabel]) {
    return ZH_LABEL_OVERRIDES[normalizedLabel];
  }
  if (isZh(locale)) {
    const localized = localizeConfigSegment(rawLabel, locale);
    if (localized !== toEnglishLabel(rawLabel)) return localized;
    const breadcrumb = localizeSchemaKeyBreadcrumb(schemaKey, locale);
    const tail = breadcrumb.split(" → ").pop();
    if (tail) return tail;
  }
  return isZh(locale)
    ? localizePhrase(humanizeKey(rawLabel), locale)
...
export function localizeConfigFieldDescription(
  ...
  if (ZH_CONFIG_PATH_DESCRIPTIONS[schemaKey]) {
    return ZH_CONFIG_PATH_DESCRIPTIONS[schemaKey];
  }
  if (ZH_DESCRIPTION_OVERRIDES[description]) {
    return ZH_DESCRIPTION_OVERRIDES[description];
  }
  if (isAutoGeneratedSchemaDescription(schemaKey, description)) {
    return ZH_CONFIG_PATH_DESCRIPTIONS[schemaKey] ?? "";
  }
...
/** Technical schema path shown under fields (hidden — users read labels, not keys). */
export function localizeConfigSchemaKeyHint(
  _schemaKey: string,
...
export function localizeConfigSectionName(
  ...
  return isZh(locale)
    ? localizeConfigSegment(section, locale)
    : humanizeKey(section);
}

export function localizeConfigScalarValue(
  schemaKey: string,
  value: unknown,
  locale: string | undefined,
): string {
  ...
  if (typeof value === "boolean") {
    return isZh(locale) ? (value ? "开启" : "关闭") : raw;
  }
  return localizeConfigSelectOption(schemaKey, raw, locale);
}
...
export function localizeConfigSelectOption(
  ...
  const normalizedSchemaKey = normalizeLookupKey(schemaKey);
  const exact =
    ZH_SELECT_OPTION_OVERRIDES[schemaKey]?.[option] ??
    ZH_SELECT_OPTION_OVERRIDES[normalizedSchemaKey]?.[option];
  if (exact) return exact;
  if (!option) return localizeConfigNoneOption(locale);
  return localizePhrase(humanizeKey(option), locale);
}
